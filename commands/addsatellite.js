...
exports.run = async (client, message, args) => {
  ...
  try {
    ...
    if (['y', 'Yes', 'yes', 'Y'].includes(collected.first().content)) {
      ...
      const createdChannel = await client.guilds.get(serverID).createChannel('satellite-portal', 'text', [{
        id: botAutoRole.id,
      ...
      const model = await DCCISatellite.findAll({
        attributes: ['guildID'],
        order: client.sequelize.col('createdAt')
      })
      const allMainIDs = Array.from(model.map(h => h.guildID))
      const allManualIDs = Array.from(await ManualSatellite.findAll({attributes: ['guildID']}).map(h => h.guildID))
      const invData = await client.fetchInvite(serverLink)
      try {
        allMainIDs.forEach(async guildID => {
          if (guildID === serverID) return
          let browseDatab = await DCCISatellite.findOne({where: {guildID: guildID}})
        ...
        try {
          ...
          try {
            allManualIDs.forEach(async guildID => {
              let browseDatab = await ManualSatellite.findOne({where: {guildID: guildID}})
            ...
            .addField('Server name:', serverName)
            .addField('Server link:', serverLink)
            .addField('Server type:', 'Satellite')
            .setTimestamp()
            .setFooter(client.config.copymark, client.user.avatarURL)
      ...
      return dmChannel.send('Exiting the prompt. Try again by triggering `>addsatellite` command in a guild I am in.\nIf issues with data perception persist, contact the developer of this bot.')
  } catch (e) {
    console.error(e)
